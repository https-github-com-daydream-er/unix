...
int     server_oriented_io(); // parallel_operation 실행 및 시간 측정
void	parallel_operation(char *client_fifo_path, char *server_fifo_path, char *io_fifo_path); // Client2Server와 parent 실행
...
void	comm_init(int id, char *fifo_path, int data[MB]); // communicate
void    send_server(char *fifo_path, int dump[MB]); // server 전달
